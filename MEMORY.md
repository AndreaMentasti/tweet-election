...
## Project: Tweet-Election Mini-Project

[LEARN:project] Research question: do tweet engagement metrics (retweet_count, favorite_count) predict party vote share in U.S. elections? Correlational framing — causal language requires identification strategy.

[LEARN:project] Data: `data/rawdata/sample_tweets.csv` (15 rows: tweet_id, user, date, text, category, retweet_count, favorite_count) + `data/rawdata/election_results.csv` (10 rows: state, year, party, candidate, votes, total_votes). Both files are PROTECTED — never overwrite.

[LEARN:workflow] Python paths: always `import config as cfg` and use `cfg.DATA_PROCESSED`, `cfg.FIGURES`, etc. Never hardcode absolute paths.

[LEARN:workflow] Stata outputs: `esttab` → `.tex` tables with `booktabs label replace`; `graph export "$root/Figures/<name>.png", replace width(2400)` for PNG at ~300 DPI.

[LEARN:project] IMMUTABLE RULES: (1) Never modify data/rawdata/ — hook-enforced + Rule 1. (2) No causal language without identification strategy — Rule 2. (3) All paths via config.py / $root — Rule 3. (4) Figures/ must be 300 DPI PNG — Rule 4. (5) Scripts run clean before commit — Rule 5. Full list: .claude/rules/project-rules.md.

[LEARN:project] Two-folder collaboration setup: code in private GitHub repo (cloned to non-Dropbox folder), data stays in Dropbox. Machine-specific path bridged by gitignored `config_local.py` (Python) and `config_local.do` (Stata) — each user creates from `.template` files. `config.py` tries `from config_local import DATA_ROOT`, falls back to repo-relative `data/`. Stata: `$root` = repo path (in source), `$data_root` = Dropbox data path (from config_local.do).
